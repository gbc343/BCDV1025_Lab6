...
class FareTransfer extends Contract {
    ...
    async EnrollCustomer(ctx, id, firstName, lastName, primaryTransit){
        ...
        await ctx.stub.putState(id, Buffer(stringify(sortKeysRecursive())));
    ...
    async GetCustomerLastFare(ctx, id) {//replaced LastTxnId with id as I felt that was more appropriate
        const customerLastFare = await ctx.stub.getState(id);
        if (!customerLastFare || customerLastFare.length === 0) {
            throw new Error(`The asset ${id} does not exist`);
        }
        const lastFare = customerLastFare.LastTxnId;
    ...
    async GetAllCustomers(ctx) {
        ...
        while(!result.done){
            const strValue = Buffer.from(result.value.toString()).toString('utf');
            let record;
            ...
            if((utils.isJSON(record) && record.docType === 'customer')){
                results.push(record);
            }
